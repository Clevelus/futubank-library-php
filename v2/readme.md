...
Почему так: плагины могут быть и под довольно древние системы, которые игнорируют PHP7 и прочие достижения цивилизации.
Поэтому чем проще, тем лучше.

## Как работают платежи

Чек-лист. Плагин должен делать три вещи:

1. иметь страницу настроек
2. уметь с помощью отдельной формочки (`<form />`) перенаправлять пользователя на платёжную страницу
3. иметь специальный url, по которому 
...
Значения `$plugininfo`, `$cmsinfo` нужны для статистики использования плагинов.  В свободной форме там указывается 
название и версия плагина и CMS. 
...
Значения `$nds` берётся из настроек плагина (см. выше). Однако в теории могут быть ситуации, когда на 
...
Внимание! Для корректной работы плагина пропишите в личном кабинете в разделе 
